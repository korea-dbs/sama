...
static void zeroPage(MemPage *pPage, int flags){
  ...
  pPage->nCell = 0;
  pPage->isInit = 1;
  // (jhpark): add overflow flag
  pPage->pDbPage->isOverflow = 0;  
}
...
static int btreeGetPage(
  ...
  assert( flags==0 || flags==PAGER_GET_NOCONTENT || flags==PAGER_GET_READONLY || flags == PAGER_GET_OVERFLOW);
  assert( sqlite3_mutex_held(pBt->mutex) );
...
static int getOverflowPage(
  ...
  */
  ...
  if( rc==SQLITE_OK ){
	...
	// (jhpark): here, we need to access OVERFLOW page!
	rc = btreeGetPage(pBt, ovfl, &pPage, (ppPage==0) ? PAGER_GET_READONLY: PAGER_GET_OVERFLOW);
...
static int accessPayload(
  ...
  if( rc==SQLITE_OK && amt>0 ){
    // (jhpark): now, we do not need overflow flag within the page
    const u32 ovflSize = pBt->usableSize - 4;  /* Bytes content per ovfl page */
    Pgno nextPage;
    nextPage = get4byte(&aPayload[pCur->info.nLocal]);

	// (jhpark): here, we need to convert overflow page number
    ...
    */
    ...
    while( nextPage ){
      ...
      // (jhpark): we ignore overflow page here
      if( nextPage > pBt->nPage && !is_ovflpage(nextPage) ) return SQLITE_CORRUPT_BKPT;
      ...
      if( offset>=ovflSize ){
        ...
        */
...

        {
          ...
          DbPage *pDbPage;
          rc = sqlite3PagerGet(pBt->pPager, nextPage, &pDbPage,(PAGER_GET_OVERFLOW));
          if( rc==SQLITE_OK ){
...
static int allocateOvflBtreePage(
  BtShared *pBt,         /* The btree */
  ...
  ** stores the total number of pages on the freelist. */
  ...
  if( n>0 ){
    ...
    fprintf(stderr, "write overflow pages; free page here!\n");
    ...
    *pPgno = pBt->nPage;
    ...
    // 1. set overflow flags
    (*ppPage)->pDbPage->isOverflow = 1;
    rc = sqlite3PagerWrite((*ppPage)->pDbPage);
    if( rc!=SQLITE_OK ){
      releasePage(*ppPage);
      *ppPage = 0;
  ...
  while( 1 ){
    ...
    ** is still writeable */
    ...
    if( spaceLeft==0 ){
      ...
      // (jhpark): we allocate overflow pages for overflow page!
      //rc = allocateOvflBtreePage(pBt, &pOvfl, &pgnoOvfl, pgnoOvfl, 0);
      //if (pOvfl->pDbPage->isOverflow != 1) return -1;

      // (jhpark): now, we calcualte overflow page number using total size of ovfl file
      /*
      struct stat file_stat;
      if (fstat(pBt->pPager->ovflFd, &file_stat) == -1) {
        fprintf(stderr, "[ERROR] Failed to get oveflow file size\n");
      }
      u32 filesz = file_stat.st_size;
      pgnoOvfl = ovfl_cur_cnt++; //set_ovfl_pgno(filesz/4096);
      pgnoOvfl = set_ovfl_pgno(pgnoOvfl);
      */
      pgnoOvfl = pBt->pPager->ovflPageCnt++;
      pgnoOvfl = set_ovfl_pgno(pgnoOvfl);

      // (jhpark): allocate new page
      rc = btreeGetUnusedPage(pBt, pgnoOvfl, &pOvfl, 1);
      rc = sqlite3PagerWrite(pOvfl->pDbPage);
      if (rc != SQLITE_OK) {
        fprintf(stderr, "[ERROR] write oveflow page error!\n");
        releasePage(pOvfl);
      }
      ...
      ** is still writeable */
      ...
      // jhpark: here, we do not maintain overflow flag within the page
      put4byte(pPrior, 0);
