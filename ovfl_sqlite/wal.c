...
struct Wal {
  ...
  // (jhpark): add overflow file descriptor
  int pOvflFd;
  //////////////////
...
// (jhpark): encode/decode overflow page number
void sqlite3WalEncodeOvflPgno(
  Wal *pWal,                      /* The write-ahead log */
  u32 ovflPgno,                      /* Overflow page number for frame */
  u8 *aFrame
){
  assert( WAL_FRAME_HDRSIZE==24 );
  sqlite3Put4byte(&aFrame[4], ovflPgno);
}

void sqlite3WalDecodeOvflPgno(
  Wal *pWal,                      /* The write-ahead log */
  u32 *ovflPgno,                      /* Overflow page number for frame */
  u8 *aFrame
){
  assert( WAL_FRAME_HDRSIZE==24 );
  *ovflPgno = sqlite3Get4byte(&aFrame[4]);
}
    ...
    if( pIter
      ...
      */
      ...
      while( rc==SQLITE_OK && 0==walIteratorNext(pIter, &iDbpage, &iFrame) ){
        ...
        // (jhpark): ignore overflow pages
        if( !is_ovflpage(iDbpage) 
            && (iFrame<=nBackfill || iFrame>mxSafeFrame || iDbpage>mxPage )){
          continue;
        }
        ...
        testcase( IS_BIG_INT(iOffset) );
      	...
      	// (jhpark): here, we write overflow pages into ovfl file;
        char ovfl_flag = 0;
        if (is_ovflpage(iDbpage)){
          write(pWal->pOvflFd, zBuf, szPage);
          ovfl_flag = 1;
        }
        if (!ovfl_flag) {
...
int is_ovflpage(u32 pgno) {
  return (pgno & (1U << 31)) != 0;
}

u32 convert_ovfl_pgno(u32 pgno) {
  return pgno & ~(1U << 31);
}

u32 set_ovfl_pgno(u32 pgno) {
  return pgno | (1U << 31);
}
...
static int walWriteOneFrame(
  ...
  // (jhpark): check overflow page here!
  if (pPage->isOverflow || is_ovflpage(pPage->pgno)) {
    sqlite3WalEncodeOvflPgno(p->pWal,pPage->pgno, aFrame);
  } 
